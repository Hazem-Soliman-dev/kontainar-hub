        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-neutral-900 dark:text-neutral-200">
              {t("home.favoritesPage.title")}
            </h1>
          </div>
          <p className="text-lg text-neutral-600 dark:text-neutral-400 max-w-2xl">
            {hasFavorites
              ? t("home.favoritesPage.description")
                  .replace("{products}", favoriteProducts.length.toString())
                  .replace("{stores}", favoriteStores.length.toString())
              : t("home.favoritesPage.emptySubtitle")}
          </p>
        </div>
      </div>
        ...
        {!hasFavorites ? (
          <div className="flex flex-col items-center justify-center gap-6 rounded-3xl border border-neutral-200 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-900 p-12 sm:p-16 text-center shadow-xl">
            ...
            <div>
              <p className="text-xl font-semibold text-neutral-900 dark:text-white mb-2">
                {t("home.favoritesPage.empty.title")}
              </p>
              <p className="text-neutral-600 dark:text-neutral-400">
                {t("home.favoritesPage.empty.description")}
              </p>
            </div>
            <div className="flex flex-wrap gap-3 justify-center">
              <Link
                href="/products"
                className="inline-flex items-center gap-2 rounded-full bg-blue-500 dark:bg-blue-600 px-6 py-3 text-sm font-semibold text-neutral-900 dark:text-neutral-200 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105"
              >
                {t("home.favoritesPage.buttons.browseProducts")}
              </Link>
              <Link
                href="/stores"
                className="inline-flex items-center gap-2 rounded-full border-2 border-blue-200 dark:border-blue-900/50 px-6 py-3 text-sm font-semibold text-neutral-900 dark:text-neutral-200 transition-all hover:bg-blue-500 dark:hover:bg-blue-600/30"
              >
                {t("home.favoritesPage.buttons.browseStores")}
              </Link>
            </div>
          </div>
        ) : !hasFilteredResults ? (
          <div className="flex flex-col items-center justify-center gap-6 rounded-3xl border border-neutral-200 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-900 p-12 text-center shadow-xl">
            <p className="text-lg font-semibold text-neutral-900 dark:text-neutral-200">
              {t("home.favoritesPage.filters.noMatch.title")}
            </p>
            <p className="text-neutral-900 dark:text-neutral-200">
              {t("home.favoritesPage.filters.noMatch.description")}
            </p>
          </div>
        ) : (
          <>
            {filteredProducts.length > 0 && (
              <section className="space-y-6">
                <h2 className="text-2xl font-bold text-neutral-900 dark:text-neutral-200">
                  {t("home.favoritesPage.sections.products.title")}{" "}
                ...
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                  {filteredProducts.map((product) => {
                    ...
                    return (
                      <article
                        key={product.id}
                        className="group relative flex flex-col gap-4 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-900 p-6 shadow-sm hover:shadow-xl transition-all duration-200 hover:-translate-y-1"
                      >
                          ...
                          {isBestSeller && "imageUrl" in product ? (
                            ...
                            <>
                              <div>
                                ...
                                <p className="mt-2 text-xs text-neutral-900 dark:text-neutral-200">
                                  {t("home.favoritesPage.supplierLabel")}{" "}
                                  {product.supplier}
                                </p>
                              </div>
                            </>
            ...
            {filteredStores.length > 0 && (
              <section className="space-y-6 mt-12">
                <h2 className="text-2xl font-bold text-neutral-900 dark:text-neutral-200">
                  {t("home.favoritesPage.sections.stores.title")}{" "}
                  <span className="text-neutral-500 dark:text-neutral-400">
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {filteredStores.map((store) => (
                    <article
                      key={store.id}
                      className="group relative flex flex-col gap-4 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 p-6 shadow-sm hover:shadow-xl transition-all duration-200 hover:-translate-y-1"
                    >
                      <div className="absolute right-6 top-6 z-10">
                        <FavoriteButton
                          store={store}
                      ...
                      >
                        ...
                        <div className="mt-auto">
                          <button className="w-full inline-flex items-center justify-center rounded-xl border-2 border-blue-200 dark:border-blue-900/50 px-4 py-2.5 text-sm font-semibold text-blue-700 dark:text-blue-400 transition-all hover:bg-blue-50 dark:hover:bg-blue-950/30">
                            {t("home.favoritesPage.buttons.viewStore")}
                          </button>
                        </div>
                      </Link>
                    </article>
