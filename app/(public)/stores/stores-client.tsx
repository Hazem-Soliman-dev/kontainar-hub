  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-50 text-neutral-900 dark:text-neutral-900">
      <main className="pb-16 sm:pb-24">
          ...
          <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
            {paginatedStores.map((store) => (
              ...
              >
                ...
                >
                  <div className="relative h-32 sm:h-40 w-full overflow-hidden rounded-xl sm:rounded-2xl border border-neutral-200 dark:border-neutral-200">
                    <Image
                      src={store.imageUrl}
                      alt={store.name}
                      fill
                    ...
                    />
                    <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-neutral-50/80 dark:from-neutral-50/80 to-transparent p-3 sm:p-4">
                      <p className="text-xs sm:text-sm font-semibold text-neutral-900 dark:text-neutral-900">
                        {store.name}
                      </p>
                    </div>
                  </div>
                  ...
                  <p className="text-xs sm:text-sm text-neutral-700 dark:text-neutral-700 line-clamp-2">{store.description}</p>

                  <div className="flex items-center justify-between">
                    <span className="text-xs text-neutral-700 dark:text-neutral-700">Login to message</span>
                    <span className="rounded-full bg-blue-500 px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-semibold uppercase tracking-wide text-neutral-50 dark:text-neutral-50 transition hover:bg-blue-600 dark:hover:bg-blue-600">
                      View profile
                    </span>
                  </div>
                </Link>
        ...
        <section className="mx-auto mt-12 sm:mt-16 flex w-full max-w-4xl flex-col items-center gap-3 sm:gap-4 rounded-2xl sm:rounded-3xl border border-blue-500/20 bg-blue-500/10 dark:bg-blue-500/10 px-4 sm:px-6 py-8 sm:py-10 text-center">
          <h2 className="text-xl sm:text-2xl font-semibold text-neutral-900 dark:text-neutral-900">
            Showcase your store on MarketHub
          </h2>
          <p className="max-w-3xl text-xs sm:text-sm text-blue-400 dark:text-blue-400">
