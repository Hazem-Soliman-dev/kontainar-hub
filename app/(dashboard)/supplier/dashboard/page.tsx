    ...
    <main className="flex min-h-screen flex-col gap-12 bg-slate-50 px-4 py-10">
      <header className="mx-auto flex w-full max-w-6xl flex-col gap-4">
        ...
        {notifications.length > 0 && (
          <aside className="flex flex-col gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-4 text-sm text-blue-900 md:flex-row md:items-center">
            <span className="font-semibold">Live order updates:</span>
            <div className="flex flex-wrap gap-2">
              {notifications.map((note, index) => (
                <span
                  key={`${note}-${index}`}
                  className="rounded-full bg-white/70 px-3 py-1 text-xs font-medium text-blue-700"
                >
                  {note}
                </span>
              ))}
